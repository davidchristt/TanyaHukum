...
export default function AdminDataModal({ onClose, onSave, initialData }) {
  ...
  // State khusus untuk error
  const [errors, setErrors] = useState({});

  useEffect(() => {
    if (initialData) {
      setFormData({ deskripsi: initialData.deskripsi });
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      // Hilangkan error dokumen jika sudah pilih file
  ...
  const handleSubmit = () => {
    ...
    if (!selectedFile) {
      newErrors.dokumen = "Harap upload dokumen terlebih dahulu!";
    }
    
    if (!formData.deskripsi.trim()) {
      newErrors.deskripsi = "Deskripsi metadata wajib diisi!";
    }
