    ...
    <>
      <div className="space-y-6 relative">
        ...
        {activeTab === "profile" && (
          <>
            <div className="space-y-2">
              <label className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest transition-colors">Nama Lengkap</label>
              <input 
                type="text" 
                value={formData.nama} 
                onChange={(e) => { setFormData({ ...formData, nama: e.target.value }); if (errors.nama) setErrors({ ...errors, nama: null }); }} 
            ...
            <div className="space-y-2">
              <label className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest transition-colors">Alamat Email</label>
              <input 
                type="email" 
            ...
            <button 
              ...
              disabled={isSaving} 
              className="px-8 py-4 bg-black dark:bg-white text-white dark:text-slate-900 rounded-2xl font-bold hover:opacity-80 transition-all active:scale-[0.98] disabled:opacity-30 flex items-center justify-center min-w-[200px]"
            >
              {isSaving ? <div className="w-5 h-5 border-2 border-white/30 dark:border-slate-900/30 border-t-white dark:border-t-slate-900 rounded-full animate-spin"></div> : "Simpan Perubahan"}
            </button>
          </>
        )}
        ...
        {activeTab === "keamanan" && (
          <>
            <div className="space-y-2">
              <label className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest transition-colors">Kata Sandi Saat Ini</label>
              <div className="relative">
                <input 
                  type={showCurrent ? "text" : "password"} 
                  value={securityForm.currentPassword} 
                  onChange={(e) => { setSecurityForm({ ...securityForm, currentPassword: e.target.value }); setErrors({}); }} 
                ...
                <button type="button" onClick={() => setShowCurrent(!showCurrent)} className="absolute right-4 top-1/2 -translate-y-1/2">
                  <img src={showCurrent ? "/icons/mataPW.svg" : "/icons/mataKetutup.svg"} alt="toggle" className="w-5 h-5 opacity-50 hover:opacity-100 transition-all" />
                </button>
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest transition-colors">Kata Sandi Baru</label>
              <div className="relative">
                <input 
                  type={showNew ? "text" : "password"} 
                  value={securityForm.newPassword} 
                  onChange={(e) => { setSecurityForm({ ...securityForm, newPassword: e.target.value }); setErrors({}); }} 
                  className="w-full px-5 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl outline-none focus:ring-4 focus:ring-black/5 dark:focus:ring-white/5 focus:border-black dark:focus:border-white transition-all font-medium pr-12 text-gray-900 dark:text-white" 
                  placeholder="••••••••" 
                />
                <button type="button" onClick={() => setShowNew(!showNew)} className="absolute right-4 top-1/2 -translate-y-1/2">
                  <img src={showNew ? "/icons/mataPW.svg" : "/icons/mataKetutup.svg"} alt="toggle" className="w-5 h-5 opacity-50 hover:opacity-100 transition-all" />
                </button>
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest transition-colors">Konfirmasi Kata Sandi Baru</label>
              <div className="relative">
                <input 
                  type={showConfirm ? "text" : "password"} 
                  value={securityForm.confirmPassword} 
                  onChange={(e) => { setSecurityForm({ ...securityForm, confirmPassword: e.target.value }); setErrors({}); }} 
                  className="w-full px-5 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl outline-none focus:ring-4 focus:ring-black/5 dark:focus:ring-white/5 focus:border-black dark:focus:border-white transition-all font-medium pr-12 text-gray-900 dark:text-white" 
                  placeholder="••••••••" 
                />
                <button type="button" onClick={() => setShowConfirm(!showConfirm)} className="absolute right-4 top-1/2 -translate-y-1/2">
                  <img src={showConfirm ? "/icons/mataPW.svg" : "/icons/mataKetutup.svg"} alt="toggle" className="w-5 h-5 opacity-50 hover:opacity-100 transition-all" />
                </button>
              </div>
            </div>
            ...
            <button 
              onClick={handleInitialSecuritySubmit} 
              disabled={isSaving} 
            ...
            >
              {isSaving ? <div className="w-5 h-5 border-2 border-white/30 dark:border-slate-900/30 border-t-white dark:border-t-slate-900 rounded-full animate-spin"></div> : "Ubah Kata Sandi"}
            </button>
          </>
        )}
      </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-300 transition-colors">
            <h3 className="text-xl font-black text-gray-900 dark:text-white mb-2 transition-colors">Konfirmasi Ubah Password</h3>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowConfirmModal(false)} className="flex-1 px-4 py-3 bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-gray-400 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-slate-700 transition-all">Batal</button>
              <button onClick={handleConfirmedSecuritySave} className="flex-1 px-4 py-3 bg-black dark:bg-white text-white dark:text-slate-900 rounded-xl font-bold hover:opacity-80 transition-all shadow-lg shadow-black/10">Ya, Ubah</button>
            </div>
          </div>
        </div>
      )}
