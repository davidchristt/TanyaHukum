...
// <-- IMPORT MODALNYA DI SINI -->
import AdminProfileModal from "../features/profileAdmin/AdminProfileModal";
...
export default function AdminSidebar({ isOpen, setIsOpen }) {
    ...
    // <-- STATE UNTUK MEMBUKA/MENUTUP MODAL PROFILE -->
    ...
    // <-- FUNGSI UPDATE DATA LOKAL BIAR FOTO/NAMA LANGSUNG BERUBAH TANPA REFRESH -->
    ...
    return (
        ...
        ${isOpen ? "w-64" : "w-20 items-center"}`}
            >
                ...
                {/* ===== PROFILE ===== */}
                <div
                    // PERUBAHAN PENTING: Jangan pakai router.push, tapi buka state modal!
            ...
            {/* RENDER POP-UP MODALNYA DI SINI */}
            <AdminProfileModal
...
function AdminMenuItem({ icon, label, isOpen, isActive, onClick }) {
    return (
        <div className="relative group/item">
            <button
                onClick={onClick}
                className={`w-full flex items-center ${isOpen ? "justify-start px-3" : "justify-center"
        ...
        ${isActive
            ...
            >
                <img
                    ...
                    alt={label}
                />
                {isOpen && <span className={`text-sm font-medium ${isActive ? "text-blue-700" : "text-gray-600"}`}>{label}</span>}
            </button>
